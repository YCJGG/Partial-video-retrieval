...
for iteration, batch in enumerate(train_loader, 0):
    ...
    fakef  = G(pf, uf)
    F_[batch_ind, :] = pf.data.cpu().squeeze()
...
for i in range(10):
    ...
    # P step
    #P = (((F_.t().mm(F_)).inverse()).mm(F_.t())).mm(B)
    ...
    B = torch.sign(Y.mm(E) + 1e-5 * Fx)

    # test
    #T = np.zeros([num_test,opt.bit],dtype = np.float32)
    H_F =  torch.zeros(num_train,4096)

    H_B = np.sign(Fx.numpy())
    ...
    for iter, batch in enumerate(test_loader, 0):
        ...
        fakef = fakef.view(-1,4096)
        #print(fakef.size())
        #print(P.size())
        H_F[batch_ind,:] = pf.cpu().squeeze()
        #H_fake = fakef.cpu().mm(P)
        #T[batch_ind.numpy(),:] = torch.sign(H_fake.cpu().data).numpy()
        #FF[batch_ind.numpy(),:] = fakef.cpu().data.numpy()
        # svm_predict = knn.predict(T[batch_ind.numpy(),:].reshape(1,-1))
        # # if predict == label:
        # #     t+=1
    ...
    # here map is acc
    T = torch.sign(H_F.mm(P))
    svm_predict = knn.predict(T.numpy().reshape(-1,64))

    s = sum(svm_predict == Label)
...
#         H_loss = (regterm1*10 )/opt.batch_size
